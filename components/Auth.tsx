...
import { X, Loader2, Mail, User as UserIcon, ShieldCheck, ArrowRight, Lock, AlertCircle } from 'lucide-react';
...
const AppleIcon = () => (
  <svg width="20" height="20" viewBox="0 0 384 512" fill="currentColor" className="flex-shrink-0">
  ...
  const handleEmailLogin = async (e: React.FormEvent) => {
    ...
    try {
      if (isRegistering) {
        ...
        const user = await api.createUser(name, email, 'email');
        onLoginSuccess(user);
      } else {
        const user = await api.login(email);
        if (user) onLoginSuccess(user);
        else setError("Conta não encontrada. Cadastre-se!");
      }
    } catch (err: any) {
  ...
  return (
    <div className="fixed inset-0 z-[120] flex items-center justify-center p-4 bg-stone-900/80 backdrop-blur-md">
      <div className="bg-white w-full max-w-md rounded-[48px] overflow-hidden shadow-2xl relative">
        <button onClick={onClose} className="absolute top-8 right-8 p-2 text-stone-300 hover:text-stone-900 transition-colors z-20">
          <X size={24} />
        </button>
        ...
        <div className="p-8 sm:p-12 pb-8">
          ...
          <div className="space-y-6 flex flex-col items-center">
            ...
            <div className="relative flex items-center justify-center w-full my-2">
              <div className="w-full border-t border-stone-100 absolute"></div>
              <span className="relative bg-white px-4 text-[9px] font-black text-stone-300 uppercase tracking-widest">ou e-mail</span>
            </div>
            ...
            <form onSubmit={handleEmailLogin} className="w-full space-y-4">
              {isRegistering && (
                <div className="group relative">
                  ...
                  <input type="text" value={name} onChange={e => setName(e.target.value)} placeholder="Seu nome" className="w-full bg-stone-50 border-2 border-stone-50 rounded-2xl pl-14 pr-6 py-4 outline-none focus:border-orange-500/20 focus:bg-white transition-all font-bold" />
                </div>
              )}
              <div className="group relative">
                <Mail className="absolute left-5 top-1/2 -translate-y-1/2 text-stone-300" size={20} />
                <input type="email" value={email} onChange={e => setEmail(e.target.value)} placeholder="E-mail" className="w-full bg-stone-50 border-2 border-stone-50 rounded-2xl pl-14 pr-6 py-4 outline-none focus:border-orange-500/20 focus:bg-white transition-all font-bold" required />
              </div>
              ...
              <button type="submit" disabled={loading} className="w-full bg-orange-500 text-white font-black py-5 rounded-full flex items-center justify-center gap-3 shadow-2xl shadow-orange-100 hover:bg-orange-600 transition-all">
                {loading ? <Loader2 className="animate-spin" /> : (
                  <>
                    <span>{isRegistering ? 'Criar Perfil' : 'Entrar'}</span>
                    <ArrowRight size={18} />
                  </>
                )}
              </button>
          ...
          <div className="mt-8 text-center">
            <button onClick={() => { setIsRegistering(!isRegistering); setError(''); }} className="text-stone-400 font-bold text-[10px] hover:text-orange-500 transition-colors uppercase tracking-widest">
              {isRegistering ? 'Já tem conta? Login' : 'Novo? Criar perfil local'}
            </button>
          </div>
          ...
          <div className="mt-10 pt-6 border-t border-stone-50 text-center space-y-2">
             <div className="flex items-center justify-center gap-2 text-[10px] font-black text-stone-400 uppercase tracking-widest">
               <Lock size={12} className="text-green-500" /> App 100% Estático
             </div>
          </div>
        </div>
      </div>
